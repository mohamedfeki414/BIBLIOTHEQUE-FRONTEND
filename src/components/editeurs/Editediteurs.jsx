...
const Editediteurs = () => {
  ...
  const getEditeurs = async (id) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/editeurs/${id}`);
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5000/api/editeurs/${id}`, editeurs);
      navigate("/editeurs");
    } catch (error) {
      console.log(error);
    }
